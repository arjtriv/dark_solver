...
async fn main() -> anyhow::Result<()> {
    ...
    // Probe RPC connectivity early so malformed ETH_RPC_URL fails closed instead of silently
    // probing a localhost fallback.
    {
        ...
        let probe_url = rpc_url.parse().map_err(|err| {
            anyhow::anyhow!("validated ETH_RPC_URL failed to parse for startup probe: {err}")
        })?;
