...
use crate::utils::abi::{decode_abi_address_array, decode_abi_usize_at};
...
async fn fetch_head_block(provider: &HttpProvider, context: &str) -> Result<u64> {
    provider
        .get_block_number()
        .await
        .map_err(|err| anyhow!("{context}: failed to fetch latest block: {err}"))
}
...
async fn enumerate_indexed_factory(
    ...
    let length_context = format!("{} factory length", factory.name);
    let total = decode_abi_usize_at(&length_result, 0, &length_context)?;
...
async fn call_array_factory(
    ...
    let decode_context = format!("{} array factory", factory.name);
    let addrs = decode_abi_address_array(&result, &decode_context)?;
...
async fn discover_event_factory_pools(
    ...
    let head = fetch_head_block(provider, factory.name).await?;
    if head == 0 {
        return Ok(Vec::new());
    }
...
async fn run_one_cycle(
    ...
    if !api_key.is_empty() {
        match fetch_head_block(&provider, "token receiver discovery").await {
            Ok(head) if head > 5000 => {
            ...
            Ok(_) => {}
            Err(e) => eprintln!("[BASESCAN] Token receiver discovery skipped: {:?}", e),
        }
    }
