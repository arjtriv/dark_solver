...
async fn resolve_protocol_contracts(categories_seen: &HashSet<String>) -> Vec<(Address, String)> {
    ...
    for factory in base_known_factories() {
        ...
        let total = match provider.call(&length_tx).await {
            Ok(result) => {
                let context = format!("{} factory length", factory.name);
                match decode_abi_usize_at(&result, 0, &context) {
                    Ok(total) => total,
                    Err(e) => {
                        eprintln!(
                            "[DEFILLAMA] {} factory length decode failed: {:?}",
                            factory.name, e
                        );
                        continue;
                    }
    ...
    for factory in base_known_array_factories() {
        ...
        match provider.call(&tx).await {
            Ok(result) => {
                let context = format!("{} array factory", factory.name);
                let markets = match decode_abi_address_array(&result, &context) {
                    Ok(markets) => markets,
                    Err(e) => {
                        eprintln!(
                            "[DEFILLAMA] {} array factory decode failed: {:?}",
                            factory.name, e
                        );
                        continue;
                    }
                ...
                let mut count = 0u32;
                for (i, addr) in markets.into_iter().enumerate() {
