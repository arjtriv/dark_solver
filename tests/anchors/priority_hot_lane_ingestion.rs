...
#[tokio::test]
async fn test_target_queue_promotes_existing_entry_to_higher_priority() {
    let (tx, mut rx) = TargetQueue::new(8);

    let promoted = Address::new([0xC1; 20]);
    let peer = Address::new([0xC2; 20]);

    assert!(tx.enqueue(promoted, TargetPriority::Dust).await);
    assert!(tx.enqueue(peer, TargetPriority::Normal).await);
    assert!(tx.enqueue(promoted, TargetPriority::Hot).await);

    let first = rx.recv().await.expect("promoted hot");
    let second = rx.recv().await.expect("peer normal");

    assert_eq!(first.address, promoted);
    assert_eq!(first.priority, TargetPriority::Hot);
    assert_eq!(second.address, peer);
