//! Anchor Test: Division/Modulo by zero must evaluate to zero.
...
fn assert_opcode_zero_divisor_yields_zero(opcode: u8) {
    ...
    let numerator = BV::new_const(&ctx, format!("anchor_numerator_{opcode}"), 256);
...
fn assert_opcode_zero_modulus_yields_zero(opcode: u8) {
    ...
    let a = BV::new_const(&ctx, format!("anchor_a_{opcode}"), 256);
    let b = BV::new_const(&ctx, format!("anchor_b_{opcode}"), 256);
    let zero = BV::from_u64(&ctx, 0, 256);
