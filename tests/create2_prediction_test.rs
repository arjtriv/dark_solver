...
#[test]
fn test_create2_init_code_audit_flags() {
    let audit =
        SymbolicMachine::audit_create2_init_code(&[0x60, 0x00, 0xf4, 0xff, 0xf5, 0xf2], 6, 6);
    assert!(audit.has_delegatecall);
    assert!(audit.has_selfdestruct);
    assert!(audit.has_nested_create2);
    assert!(audit.has_callcode);
    assert!(audit.suspicious());
}

#[test]
fn test_create2_symbolic_prediction_is_160bit_address_form() {
    let cfg = Config::new();
    let ctx = Context::new(&cfg);
    let solver = Solver::new(&ctx);
    let machine = SymbolicMachine::new(&ctx, &solver, None);
    ...
    let salt = BV::new_const(&ctx, "create2_salt_sym", 256);
    let init_hash = BV::new_const(&ctx, "create2_hash_sym", 256);
